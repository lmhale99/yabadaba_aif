...
# yabadaba imports
from yabadaba.tools import aslist, iaslist
from yabadaba.database import Database
from yabadaba.record import recordmanager, load_record, Record

class AIFDatabase(Database):

    def __init__(self,
                 host: Union[str, Path]):
        """
        Initializes a connection to a database of AIF records stored in a
        local directory.
    ...
    def cache(self,
        ...
        if addnew is True:
            ...
            filenames = set([fname.stem for fname in Path(self.host, style).glob(f'*.aif')])
            ...
            if len(newnames) > 0:
                ...
                for name in newnames:
                    fname = Path(self.host, style, f'{name}.aif')
                    record = load_record(style, aif=fname)
    ...
    def get_records(self, 
        ...
        **kwargs : any, optional
        ...
        if len(df) > 0:
            for name in df.name:
                fname = Path(self.host, style, f'{name}.aif')
                records.append(load_record(style, aif=fname, database=self))
    ...
    def count_records(self,
        ...
        **kwargs : any, optional
        ...
        if len(kwargs) == 0:
            count = len([i for i in Path(self.host, style).glob(f'*.aif')])
    ...
    def add_record(self,
        ...
        fname = Path(style_dir, f'{record.name}.aif')
        if fname.is_file():
        ...
        # Save record
        record.build_aif(output_file=fname)
    ...
    def delete_record(self,
        ...
        # Issue a ValueError for competing kwargs
         ...
         # Delete record file
        fname = Path(self.host, record.style, f'{record.name}.aif')
        if fname.is_file():
            fname.unlink()
        else:
